...
import numpy as np
from h5stitch2D import chunkIt, getfnsp4, getfns, fields2D, h5fields2D, h5fields2Dpar2, getTimes
import gzip
...
fns = getfnsp4(folder)[::2]
#h5fields2D(folder, fld_ids = ['Ez'])
#h5fields2Dpar2(folder, fld_ids = ['Ez'])

# Load in the data
divsp = 1 # Integer divisor by which to reduce the spatial resolution
data = fields2D(fns, fld_ids = ['Ez'])
times = data['times']
fns = data['filenames']
xgv = data['xgv'][::divsp]*1e4
zgv = data['zgv'][::divsp]*1e4
